...
import models.attacker.learning.rollout_collection # To setup the merged stepping during training
from models.attacker.env.hover_env import QuadXHoverAgiliciousAttEnv
...
for hyperparam in H:
    ...
    config['run_name'] = f"ATT_arch={'A' if sum(hyperparam['architecture']) == 128 else 'B'}, lr={round(hyperparam['learning_rate'],6)}, bs={hyperparam['batch_size']}, gamma=.99, T={hyperparam['n_steps']}"
    config['save_path'] = f'models/attacker/runs/{datetime.now().strftime("%Y-%m-%d/%H-%M-%S")}'
    ...
    def make_env(rank, seed=0):
        def _init():
            env = Monitor(QuadXHoverAgiliciousAttEnv(sparse_reward=False)) # Wrap in Monitor to get training metrics
    ...
    eval_env = Monitor(QuadXHoverAgiliciousAttEnv(
        sparse_reward=False, 
