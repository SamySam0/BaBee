...
config = {
    ...
    "model": 'Agilicious',
    "save_path": f'models/attacker/runs/{datetime.now().strftime("%Y-%m-%d/%H-%M-%S")}',
    "nb_parallel_env": 3,
}
...
for hyperparam in H:
    ...
    config['run_name'] = f"ATT_arch={'A' if sum(hyperparam['architecture']) == 128 else 'B'}, lr={round(hyperparam['learning_rate'],6)}, bs={hyperparam['batch_size']}, gamma=.95, T={hyperparam['n_steps']}"
