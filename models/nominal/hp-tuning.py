...
for hyperparam in H:
    ...
    config['run_name'] = f"NOM_arch={'A' if sum(hyperparam['architecture']) == 128 else 'B' if sum(hyperparam['architecture']) == 128+64 else 'C'}, lr={round(hyperparam['learning_rate'],6)}, bs={hyperparam['batch_size']}, gamma=.99, T={hyperparam['n_steps']}"
    config['save_path'] = f'models/nominal/runs/{datetime.now().strftime("%Y-%m-%d/%H-%M-%S")}'
