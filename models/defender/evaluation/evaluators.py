...
def visual_evaluation(nominal_model, attacker_model, defender_model, eval_env, n_eval_episodes):
...
def evaluate_to_plot_2d(nominal_model, attacker_model, defender_model, eval_env, n_eval_episodes, hover_point, plot_path):
    # Save errors when under defence and nominal only
    all_errors_to_hover_de = np.zeros((n_eval_episodes, 402*3)) # *3 for 120Hz
    all_errors_to_hover_no = np.zeros((n_eval_episodes, 402*3)) # *3 for 120Hz

    # Save average error to hover point over time
    # For both nominal and defender, for comparison
    for run_id in range(n_eval_episodes):

        # -- Under Defence --
        # Get X,Y,Z positions at each timestep
        xs, ys, zs = get_results_of_single_sim(nominal_model, attacker_model, defender_model, eval_env)
        ...
        all_errors_to_hover_de[run_id] = errors
    ...
    plot_distance_to_hover(
        ...
        errors = np.mean(all_errors_to_hover_de, axis=0),
        std = np.std(all_errors_to_hover_de, axis=0),
        reference = (
...
def evaluate_to_plot_3d(nominal_model, attacker_model, defender_model, eval_env, hover_point, plot_path):
    
    # Run one simulation and get X,Y,Z positions at each timestep
    xs_de, ys_de, zs_de = get_results_of_single_sim(nominal_model, attacker_model, defender_model, eval_env)
