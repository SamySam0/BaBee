...
attacker = PPO.load('checkpoints/attacker_model')

# Rollout collection
def rollout_collection(self, env, callback, rollout_buffer, n_rollout_steps, nominal=nominal, attacker=attacker):
    ...
    while n_steps < n_rollout_steps:
        ...
        if isinstance(self.action_space, spaces.Discrete):
            ...
            actions_defender = actions_defender.reshape(-1, 1) 
    ...
    with torch.no_grad():
        ...
        values_defender = self.policy.predict_values(obs_as_tensor(new_obs, self.device))  # type: ignore[arg-type]

    rollout_buffer.compute_returns_and_advantage(last_values=values_defender, dones=dones)
